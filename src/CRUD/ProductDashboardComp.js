...
const ProductDashboardComp = () => {
    ...
    const fetchData =() => {
        axios.get(`http://localhost:8888/foodMenu`).then((response) => {
    ...
    const DeleteProduct = (id) =>{
        if(window.confirm(`Are you sure to delete Id:${id}`)){
            axios.delete(`http://localhost:8888/foodMenu/${id}`).then(()=>{
    ...
    return (
        <div>
            ...
            <Link to="/Maindashboard/AdminAdd" className='btn btn-primary' ><AddIcon /> &nbsp;ADD</Link>
            <Link to="/Maindashboard"  className='btn btn-primary' style={{float:"right"}}>&nbsp;Back</Link><br />
